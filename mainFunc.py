...
def nonLinearConfInt(func, param1, param2, dataY, **kwargs):
    ...
    argsIterate = np.array([*args])

    # parse  tuples from arguments
    count = 0
    for i in range(numberMeas):
        argTuple = ()
        for j in range(len(argsIterate)):
            argTuple += (argsIterate[j][i],)
        for k in range(xLength):
            for l in range(yLength):
                #print(param1_grid[k][l])
                #print(param2_grid[k][l])
                modelValues[k, l, i] = func(param1_grid[k][l], param2_grid[k][l], *argTuple)
                #print(modelValues[k,l,i])
                if (math.isnan(modelValues[k, l, i])):
                    modelValues[k, l, i] = modelValues[k, l-1, i]
                count += 1
